  ...
  if (!user) {
    return (
      <div className="text-center py-12">
        ...
        <Link to="/login" className="inline-block bg-[#0D0D0D] text-white px-6 py-3 rounded hover:bg-[#2F2B2B]">
          Go to Login
        </Link>
      </div>
    )
  ...
  if (favs.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <Link to="/shop" className="inline-block bg-[#0D0D0D] text-white px-6 py-3 rounded hover:bg-[#2F2B2B]">
          Continue Shopping
        </Link>
      </div>
    )
  ...
  return (
    <div>
      ...
      <p className="text-gray-600 mb-8">
        You have {favs.length} favorite item{favs.length !== 1 ? 's' : ''}
      </p>
      ...
      <div className="grid grid-cols-3 gap-6">
        {favs.map(f => (
          <div key={f.id} className="bg-white rounded shadow overflow-hidden">
            ...
            <div className="p-4">
              <Link to={`/product/${f.id}`}>
                <h3 className="font-semibold text-sm line-clamp-2 hover:text-[#0D0D0D]">
                  {f.title}
                </h3>
              </Link>
              <p className="text-[#0D0D0D] font-bold mt-2">${f.price}</p>
              <div className="mt-3 flex gap-2">
                <Link to={`/product/${f.id}`} className="flex-1 bg-[#0D0D0D] text-white text-sm py-2 rounded hover:bg-blue-700 text-center">
                  View
                </Link>
                <button
